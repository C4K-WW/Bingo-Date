...
(function () {
  document.forms.register1.noValidate = true; // Disable HTML5 validation - using JavaScript instead
  ...
  $('form').on('submit', function (e) {      // When form is submitted    
    ...
    if (!validateRegisterPasswordInput()) {          // Call validatePassword(), and if not valid
      showErrorMessage(document.getElementById('registerPasswordInput')); // Show error message
      valid.registerPasswordInput = false;           // Update the valid object - this element is not valid
    } else {                            // Otherwise remove error message
      removeErrorMessage(document.getElementById('registerPasswordInput'));
  ...
  // function validateAgeConsent() {
  //   var ageConsent   = document.getElementById('agesconsent');
  ...
  //     valid = ageConsent.checked;          // Update valid: is it checked/not
  //     if (!valid) {                            // If not, set the error message
  //       setErrorMessage(ageConsent, 'You need to be age over 18 to join this site\' consent');
  //     }
  //   }
  ...
  //   if (!valid) {
  //     setErrorMessage(membershipIlikeToMeet, 'Please make sure your about me does not exceed 250 characters');
  //   }
  //   return valid;
  // }
