...
(function () {
  document.forms.membership.noValidate = true; // Disable HTML5 validation - using JavaScript instead
  // document.forms.register2.noValidate = true;
  ...
  $('form').on('submit', function (e) {      // When form is submitted    
    ...
    if (!validateMembershipAboutMe()) {                // Call validateBio(), and if not valid
      showErrorMessage(document.getElementById('membershipAboutMe')); // Show error message
      valid.membershipAboutMe = false;                 // Update valid object - this element is not valid
    } else {                             // Otherwise remove error message
      removeErrorMessage(document.getElementById('membershipAboutMe'));
    }
     
  // like to meet (you could cache liketomeet input in variable here)
   if (!validateMembershipIlikeToMeet()) {                // Call validateBio(), and if not valid
      showErrorMessage(document.getElementById('membershipIlikeToMeet')); // Show error message
      valid.membershipIlikeToMeet = false;                 // Update valid object - this element is not valid
    } else {                             // Otherwise remove error message
      removeErrorMessage(document.getElementById('membershipIlikeToMeet'));
    }
    ...
    // Age consent (you could cache parent-consent in variable here)
    if (!validateAgesConsent()) {     // Call validateParentalConsent(), and if not valid
      showErrorMessage(document.getElementById('agesconsent')); // Show error message
      valid.agesConsent = false;      // Update the valid object - this is not valid
    } else {                             // Otherwise remove error message
      removeErrorMessage(document.getElementById('agesconsent'));
    }
  ...
  // IF USER IS UNDER 19, CHECK THAT PARENTS HAVE TICKED THE CONSENT CHECKBOX
  // Dependency: birthday.js (otherwise check does not work)
  function validateAgesConsent() {
    var agesConsent   = document.getElementById('agesconsent');
    var ageConsentContainer = document.getElementById('agesConsentContainer');
    var valid = true;                          // Variable: valid set to true
    if (ageConsentContainer.className.indexOf('hide') === -1) { // If checkbox shown
      valid = agesConsent.checked;          // Update valid: is it checked/not
      if (!valid) {                            // If not, set the error message
        setErrorMessage(ageConsent, 'You need your parents\' consent');
      }
    }
    return valid;                               // Return whether valid or not
  }

  // Check if the bio is less than or equal to 250 characters
  function validateMembershipAboutMe() {
    var membershipAboutMe = document.getElementById('membershipAboutMe');
    var valid = membershipAboutMe.value.length <= 250;
    if (!valid) {
      setErrorMessage(membershipAboutMe, 'Please make sure your about me does not exceed 250 characters');
    }
    return valid;
  }

  // Check if the bio is less than or equal to 250 characters
  function validateMembershipIlikeToMeet() {
    var membershipIlikeToMeet = document.getElementById('membershipIlikeToMeet');
    var valid = membershipIlikeToMeet.value.length <= 250;
    if (!valid) {
      setErrorMessage(membershipIlikeToMeet, 'Please make sure your words does not exceed 250 characters');
    }
    return valid;
  }
